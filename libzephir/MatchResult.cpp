    ...
    void MatchResult::_update() {
        ...
        if (m_action.has_value() && m_resource.has_value()) {
            ...
            this->m_partial = std::make_unique<PartialPolicy>(
                this->m_policy._version,
                this->m_policy._effect,
                m_action.has_value() ? std::nullopt : this->m_policy._actions,
                m_resource.has_value() ? std::nullopt : this->m_policy._resources
            );
        }
    }
}
